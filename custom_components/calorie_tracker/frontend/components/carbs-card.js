class CarbsGaugeCard extends HTMLElement {
  ...
  this._applyCarbsStyles(wrapper);
    ...
    try {
      ...
      let computedMax = Math.ceil(((weightLb || 0) * 1.36) / 10) * 10; // default: user's weight in lbs * 1.36 rounded up to nearest 10 (display range will be weight * 1.5)
      if (this.max !== null) {
  ...
  this._carbsMin = this.min;
  this._carbsMax = this.max;
